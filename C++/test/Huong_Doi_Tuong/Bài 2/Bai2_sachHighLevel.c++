...
/* void docfile(ifstream &filein, sach &book){
	getline(filein,book.tensach, ',');
	getline(filein,book.tongsosach, ',');
	getline(filein,book.sachchomuon, ',');
}
//doctensachvathongtintuongung
void doc_files(ifstream &filein , sach &danhsachsach){
	//vong lap lap den cuoi file thi dung;
	while(filein.eof()==false ){
		//doc thong tin cua 1 tieu de sach
		sach a;
		docfile(filein,a);
	}
} */
...
void nhapthongtin(){
	...
	cin>>n;
	//luu thong tin ten sach tu file
	for(int i=1;i<=n;i++){
		file>>a[i].tensach;
	}
	for(int i=1;i<=n;i++){
		cout<<"Nhap thong tin cua sach : "<<a[i].tensach<<endl;
		a[i].nhap();
		cout<<endl;
	}
	file.close();
...
void sachconlai(){
	ofstream filenhan;
	cout<<"Nhap tieu chi sach con lai can tim kiem : ";x`
	cin>>m;
